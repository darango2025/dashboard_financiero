...
async function syncAllEndpoints() {
  ...
  let dashboardData;
  try {
    dashboardData = buildEnhancedDashboard(results);
  } catch (buildErr) {
    console.error('[Orchestrator] Dashboard build failed:', buildErr.message);
    dashboardData = { error: buildErr.message, source: 'rsales' };
  }
...
function buildEnhancedDashboard(results) {
  let { receivables = [], customers = [], sellers = [], orders = [], 
  ...
  stocks = stocks.filter(st => st && typeof st === 'object');
  products = products.filter(p => p && typeof p === 'object');
  prices = prices.filter(pr => pr && typeof pr === 'object');
  routes = routes.filter(r => r && typeof r === 'object');
  payment_methods = payment_methods.filter(pm => pm && typeof pm === 'object');
  factors = factors.filter(f => f && typeof f === 'object');
